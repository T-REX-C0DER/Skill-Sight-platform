(function(){
  const questions = window.pythonQuestions || [];
  ...
  if(missingElements.length){
    console.error('python-test: missing required DOM elements:', missingElements);
  }
  ...
  function render(){
    ...
    try{
      ...
      console.error('python-test: render error', err);
    }
  }
  ...
  function scoreForPersist(){
    // compute current visible score (number of correct answers)
    let s = 0; questions.forEach((q,i)=>{ if(q.answerIndex === answers[i]) s++; });
    if(window.TEST_FAILED) s = 0; return s;
  }

  // persist will be called when results are shown
  ...
  if(timerEl){
    ...
    console.warn('python-test: timer element not found');
  }
})();
